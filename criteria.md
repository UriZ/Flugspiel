...
| 11 | No comment in the changed file contradicts what shipped **or another comment in the same file** | Medium -> **must-fix on a second appearance in the same file** | Distinct from row 10, which covers *"the amendment did not land at the call site"*. This covers *"a comment elsewhere in the same file now asserts the opposite of what shipped"* — which **three agents walked past** on `lif.py`: `10dc238` existed to remove a false claim from that file and left `lif.py:149` ("nothing pins it (#24)") standing 40 lines above, in `PARTITIONS`' own docstring, i.e. the text someone reads at the moment they edit the constant. Grep the whole changed file for claims your change falsified, not just the lines you touched. **And a companion clause, because row 11 as written is a TEXT rule and cannot reach a symbol nothing calls: every exported symbol whose comment claims it enforces, authorises or is the sole source of something must be shown to have a call site.** One grep catches the whole class. Two independent whole-file reads of `src/viz/` — a feature gate's and a second gate's — found **none** of the three reachability defects in it (#52, #55, #56), because both readers were checking comment-against-code *within* a file and neither asked whether the symbol is reached. That is a rule gap, not careless reading. It is also the project's third instance of the shape after #17's unreachable `_MIN_RATE` and #48's mis-scoped `--contract` check, and in all three **the shipped state IS the inert case**, so a mutation battery has nothing to mutate. **And there are TWO shapes here, not one — the clause above covers only the first.** (a) A guard that **never executes**, like #17's `_MIN_RATE` sitting after a `read()` that fills its buffer, or an exported symbol with no call site. (b) A guard that **executes on every call but whose condition an upstream sanitiser has made unsatisfiable** — `lif.py`'s `_check_inject` non-finite arm runs every step, yet `encoder.py`'s `np.nan_to_num` strips NaN, +inf and -inf before `step()` is reached, so via encoder -> server -> brain the branch cannot be taken. **A reachability grep finds the first and not the second.** Shape (b) is legitimate defence-in-depth for other callers; what makes it worth recording is that such a guard has been demonstrated by neither a test nor a production path. **Two defect classes, and the second was missed by a rule written for the first**: a claim that was *true when written* and invalidated by a later change, **and** a claim that was *never true*, introduced by the same commit that changed the code, contradicting **another comment three lines above it** rather than the code. `lif.py` carried one of each, eight lines apart. **Read the whole file, not the hunk** — both were found only by a grep mandated after the previous one was found, and a targeted fix reads a window |
